...
Output: ported `heroCarousel.svelte.ts` factory; `HeroCarousel.svelte`.

NOTE — verification model for THIS repo: there is NO test runner (this project intentionally stripped _three's vitest/CI harness — see 02-CONTEXT). Verify with `pnpm check` (svelte-check) and `pnpm build`, plus grep assertions over the SOURCE files for the exact UI-SPEC strings. Do NOT add vitest, a `test` script, or `*.test.ts` files. Do NOT port _three's `*.test.ts` files — port ONLY the `heroCarousel.svelte.ts` factory itself.
INFO: `import { createHeroCarousel } from '$lib/heroCarousel.svelte'` (no `.ts`) is correct — the file is `heroCarousel.svelte.ts` and Svelte 5 resolves the runes module by its `.svelte` virtual extension. Leave that import shape as-is.
</objective>
...
From src/lib/state/motion.svelte.ts: `export const motion: { readonly prefersReducedMotion: boolean };` — single source of truth; never matchMedia.
...
<!-- IGNORE the *.test.ts files in _three — this repo has no test runner; do NOT port them. -->
...
<task type="auto">
  <name>Task 1: Port the heroCarousel rune factory</name>
  <read_first>
  ...
  <action>
    ...
    Scrub any forward-phase / iframe-specific comment references that don't apply to this poster-only variant ONLY if they mention components not in this repo; keep the behavioral comments. Do NOT port the matching `*.test.ts` file (this repo has no test runner). Do NOT mention AI assistants anywhere.
  </action>
  <acceptance_criteria>
    ...
    - `grep -ciE "claude|gemini|copilot|ai assistant" src/lib/heroCarousel.svelte.ts` returns 0.
    - No test file was created: `test ! -f src/lib/heroCarousel.svelte.test.ts`.
    - `pnpm check` reports no errors for heroCarousel.svelte.ts.
  </acceptance_criteria>
  <verify>
    <automated>cd "C:/Users/Mkaru/Documents/Hello_World/hugginface_profile/Websites/michelle_ngo_websites/michelle_ngo_six" && grep -F "HERO_CAROUSEL_INTERVAL_MS = 7000" src/lib/heroCarousel.svelte.ts && grep -F "isAutoAllowed" src/lib/heroCarousel.svelte.ts && [ ! -f src/lib/heroCarousel.svelte.test.ts ] && pnpm check</automated>
  </verify>
  <done>The 7000ms auto-advance factory with userPaused latch + per-tick gate + SSR no-op is in this repo; no test file created; pnpm check clean.</done>
</task>

<task type="auto">
  <name>Task 2: Build HeroCarousel.svelte (poster+title slides, dots, crossfade, pause, reduced-motion)</name>
  <read_first>
  ...
  <behavior>
    ...
    - With prefers-reduced-motion: no auto-advance (isAutoAllowed returns false) and crossfade is instant; dots still change the active slide.
  ...
  <action>
    ...
    - Slide stack: one absolutely-positioned layer per slide, crossfaded by `opacity` over 600ms `var(--ease-cinematic)`. REQUIRED gating mechanism (single approach — do NOT use Tailwind `motion-safe:` variants, which read the media query directly and bypass the locked single-source `motion` rune): bind `class:motion-ok={!motion.prefersReducedMotion}` onto each slide layer (or the stack root) and apply the opacity transition only under `.motion-ok` in the `<style>` block, so reduced-motion swaps are instant (no transition). Inactive layers `opacity-0 pointer-events-none`. NO translate/slide. (Optional Ken-Burns scale(1.04->1) is OUT for v1 unless trivial — skip to protect LCP.)
    - Per slide:
    ...
    Reduced-motion (UI-SPEC): the rune already blocks auto-advance via `isAutoAllowed`. ALSO gate the crossfade via the `class:motion-ok={!motion.prefersReducedMotion}` mechanism above (NOT `motion-safe:`) so switches are instant under reduced motion. Render slide 0 statically by default (activeSlide starts at 0). Do NOT call `matchMedia`. Do NOT override `:focus-visible`. NO iframe, NO `<canvas>`/WebGL.
  ...
  <acceptance_criteria>
    ...
    - `grep -F "createHeroCarousel" src/lib/components/HeroCarousel.svelte` and `grep -F "startAuto" src/lib/components/HeroCarousel.svelte` match.
    ...
    - `grep -F "motion.prefersReducedMotion" src/lib/components/HeroCarousel.svelte` matches AND `grep -F "motion-ok" src/lib/components/HeroCarousel.svelte` matches (rune-bound crossfade gate) AND `grep -c "motion-safe:" src/lib/components/HeroCarousel.svelte` returns 0.
    - `grep -c "matchMedia" src/lib/components/HeroCarousel.svelte` returns 0 AND `grep -c "iframe" src/lib/components/HeroCarousel.svelte` returns 0 AND `grep -c "canvas" src/lib/components/HeroCarousel.svelte` returns 0.
    - `grep -F "oklch(0.16 0 0 / 0.92)" src/lib/components/HeroCarousel.svelte` matches (scrim).
    - `pnpm check` reports no errors for HeroCarousel.svelte AND `pnpm build` exits 0.
  </acceptance_criteria>
  <verify>
    <automated>cd "C:/Users/Mkaru/Documents/Hello_World/hugginface_profile/Websites/michelle_ngo_websites/michelle_ngo_six" && [ "$(grep -c 'fetchpriority=\"high\"' src/lib/components/HeroCarousel.svelte)" -ge 1 ] && grep -F "motion-ok" src/lib/components/HeroCarousel.svelte && [ "$(grep -c 'motion-safe:' src/lib/components/HeroCarousel.svelte)" = "0" ] && [ "$(grep -c 'iframe' src/lib/components/HeroCarousel.svelte)" = "0" ] && [ "$(grep -c 'matchMedia' src/lib/components/HeroCarousel.svelte)" = "0" ] && pnpm check && pnpm build</automated>
  </verify>
  <done>HeroCarousel renders reel-first poster+title slides as /watch links with exactly one eager+high poster, neutral dots (aria-label/aria-current), 7000ms/600ms crossfade auto-advance that pauses on hover/focus, and a static-but-navigable reduced-motion variant gated by the motion-ok class — no iframe/canvas/matchMedia/motion-safe; pnpm check + pnpm build green.</done>
</task>
...
- Auto-advance + crossfade are gated solely on `motion.prefersReducedMotion` (via `isAutoAllowed` + the `motion-ok` rune-bound class, NOT `motion-safe:`) plus hover pause.
</verification>
...
<output>
After completion, create `.planning/phases/02-homepage-rails/02-03-SUMMARY.md` documenting: the slide set construction (`[reel, ...getHeroSlides()]`), the pause-flag wiring into `isAutoAllowed`, and the crossfade gating mechanism (the `motion-ok` rune-bound class) for Plan 04 + Phase 5.
</output>
</content>
